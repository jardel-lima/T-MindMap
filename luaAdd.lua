...
local TEXT = ''
local CHAR = ''
local file = 'Temas.txt'
local txt = '' 
local visible_at = false
local img=canvas:new('media/addtema.png')-- carrega a imagem 
...
local function nclhandler_at(evt)

	if evt.class == 'ncl' then   
		if evt.type == 'attribution' then 
		   if evt.name == 'visi_at' then
...
event.register(nclhandler_at)

local function keyHandler_at (evt)
	...
	if (key == 'ENTER') then
		...
		event.timer(100, function()
			selat.action = 'start'; event.post(selat)
			selat.action = 'stop';  event.post(selat)
		end)
